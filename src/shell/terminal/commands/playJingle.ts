 ...
 * Plays a silent PCM WAV stream and sleeps for 250ms (0.25s) to allow sound card DAC hardware amplifier to fully warm up.
 */
...
export function handlePlayJingle(): string {
  ...
  if (platform === 'win32') {
    ...
    // Silent PCM header, start background silent playback, sleep 250ms to warm hardware DAC, then play jingle
    ...
    command = `powershell -c "$s = [System.IO.MemoryStream]::new([byte[]](${silentWavBytes})); $wp = New-Object Media.SoundPlayer $s; $wp.Play(); [System.Threading.Thread]::Sleep(250); $p = New-Object Media.SoundPlayer '${escaped}'; $p.Load(); $p.PlaySync()"`;
